((buster, require) ->
    ...
    fakeForm = elements: [
        ...
        }
        {
            ...
            name: 'radioGroup1'
            type: 'radio'
            value: 'rgroup value 1'
        }
        {
            name: 'radioGroup1'
            type: 'radio'
            value: 'rgroup value 2'
            checked: true
        }
        {
            name: 'radioGroup1'
            type: 'radio'
            value: 'rgroup value 3'
        }
        {
            name: 'checkboxGroup1'
            type: 'checkbox'
    ...
    for i of fakeForm.elements
        continue
    buster.testCase 'cola/dom/formToObject',
        formToObject:
            'should extract values from non-grouped form inputs': ->
                obj = form.getValues(fakeForm)
            ...
            'should extract arrays from grouped form inputs': ->
                obj = form.getValues(fakeForm)
                ...
                assert.equals [
                    'group value 1'
                    'group value 2'
                ], obj.checkboxGroup2, 'checkbox group with multiple checked A'
                assert.equals [
                    'group value 2'
                    'group value 3'
                ], obj.checkboxGroup3, 'checkbox group with multiple checked B'
                assert.equals ['option value 2'], obj.multiselect1, 'multiple select with only one item selected'
                assert.equals [
                    'option value 2'
                    'option value 3'
                ], obj.multiselect2, 'multiple select with many items selected A'
                assert.equals [
                    'option value 1'
                    'option value 2'
                ], obj.multiselect3, 'multiple select with many items selected B'
                return

            'should skip over un-named and non-value elements': ->
                obj = form.getValues(fakeForm)
                refute 'no-value' of obj
                refute 'no-name' of obj
                refute '' of obj
                return
