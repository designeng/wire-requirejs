        ...
        (options) ->
            ...
            return syncDataDirectly = (source, dest, provide, type, api) ->
                if 'sync' is type and api.isBefore()
                    ...
                    try
                        if provide
                            throw new Error('syncModel: provider doesn\'t have `properties()`.')    unless typeof source.properties is 'function'
                            add providers, synced
                            add consumers, synced    if options.providersAreConsumers
                            forEach consumers, (consumer) ->
                                source.properties (item) ->
                            ...
                            if typeof source.update is 'function'
                                forEach providers, (provider) ->
                                    provider.properties (item) ->
                                        synced.update item
                                        return
